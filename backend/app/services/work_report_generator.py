...
class WorkerType(Enum):
    """Type of worker for hour distribution."""
    WORKER = "worker"  # Single worker, max 8h per day
    TEAM = "team"      # Team, no daily limit
...
class WorkReportConfig:
    ...
    worker_type: WorkerType = WorkerType.WORKER  # Default to single worker
...
class WorkReportGenerator:
    ...
    def generate_tasks_from_analysis(
        ...
        # Apply daily hour limit for single worker (max 8h per day)
        if config.worker_type == WorkerType.WORKER:
            work_days = self._get_work_days(config.start_date, config.end_date)
            tasks = self._distribute_tasks_with_daily_limit(tasks, work_days, max_hours_per_day=8.0)
...
Make tasks specific to the codebase type and languages used."""
    ...
    def _distribute_tasks_with_daily_limit(
        self,
        tasks: List[WorkTask],
        ...
        """
        Redistribute tasks to respect daily hour limit for single worker.

        If a task has more hours than allowed per day, split it across multiple days.
        """
        if not work_days:
            return tasks
        ...
        for task in tasks:
            ...
            while remaining_hours > 0 and day_idx < len(work_days):
                ...
                if available_today <= 0:
                    ...
                    if day_idx >= len(work_days):
                        break
                    available_today = max_hours_per_day

                # Allocate hours for this day
                hours_for_this_entry = min(remaining_hours, available_today)

                distributed_tasks.append(WorkTask(
                    date=work_days[day_idx].strftime('%d.%m.%Y'),
                    activity=task.activity,
                    result=task.result,
                    hours=round(hours_for_this_entry, 1)
                ))

                remaining_hours -= hours_for_this_entry
                hours_used_today += hours_for_this_entry
